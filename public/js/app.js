document.addEventListener('DOMContentLoaded', () => {
  ...
  const renderGallery = () => {
    ...
    filteredItems.forEach((item, index) => {
      ...
      const rawImageSrc = String(item.filename || '');
      const imageSrc = rawImageSrc.startsWith('http') || rawImageSrc.startsWith('/')
        ? rawImageSrc
        : `/img/${rawImageSrc}`;
      const fallbackSrc = `data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='400' height='400' viewBox='0 0 400 400'><rect width='100%' height='100%' fill='%23EBE6E4'/><text x='50%' y='50%' font-family='sans-serif' font-size='18' fill='%237A5C58' text-anchor='middle'>%D0%98%D0%9D%D0%94%D0%98%D0%90%D0%9B</text></svg>`;

      div.innerHTML = `
        <img src="${imageSrc}" alt="${item.description}" loading="lazy" decoding="async" onerror="this.onerror=null; this.src='${fallbackSrc}';">
